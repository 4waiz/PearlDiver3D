...
// FIX: Use a deeper, more contrasting ocean color
const waterMat = new THREE.MeshPhongMaterial({ color: 0x005f73, transparent:true, opacity:0.85, side:THREE.DoubleSide, shininess:95, specular:0xffffff });
...
function animate(){
  ...
  if(state.running && !state.paused){
    ...
    for(let i=0;i<posAttr.count;i++){
      ...
      // FIX: Increase wave amplitude for better visibility
      const y = Math.sin((x+now*0.05)*0.01)*0.8 + Math.cos((z+now*0.04)*0.01)*0.6;
      posAttr.setY(i, y);
    }
